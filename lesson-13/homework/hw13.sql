...
--1
select cast(EMPLOYEE_ID as varchar)+'-'+	FIRST_NAME+' '+LAST_NAME as ID_FullName from Employees

--2
select replace( PHONE_NUMBER ,124, 999) from Employees

--3
select FIRST_NAME, LEN(FIRST_NAME) as Len_FirstName from Employees
where FIRST_NAME like '[ajm]%'
order by FIRST_NAME

--4
select distinct e1.EMPLOYEE_ID as ManagerID, e1.FIRST_NAME+e1.LAST_NAME as ManagerFullName, sum(e1.SALARY) as TotalSalary
from Employees e1
...
--8
select coalesce( cast(id as Varchar),ssn,passportid,itin) as NonNullColumn from person
--1 medium
select Left(fullName,CHARINDEX(' ',fullName)-1) as FirstName,
Substring(FullName,CHARINDEX(' ',fullName),len(FullName)-(CHARINDEX(' ',fullName)+ len(FullName)-CHARINDEX(' ',FullName,CHARINDEX(' ',FullName)+1))) as MiddleName, 
right(FullName,len(FullName)-CHARINDEX(' ',FullName,CHARINDEX(' ',FullName)+1)) as LastName from Students
select FullName from Students

--2 medium

select or1.CustomerID,or1.OrderID, Or1.DeliveryState from Orders Or1
join (select distinct CustomerID from Orders
where DeliveryState='CA') c
on or1.CustomerID=c.CustomerID
...
concat(100 * sum(case when HIRE_DATE<=dateadd(year,-3,getdate()) then 1
else 0 end)/count(*),'%') PercentOver3Year
from Employees
...
SELECT 
    ...
    (SELECT TOP 1 SpacemanID 
     ...
     ORDER BY MissionCount DESC) AS MostExperiencedSpacemanID,
    (SELECT TOP 1 SpacemanID 
     FROM Personal p2 
     WHERE p2.JobDescription = p.JobDescription 
     ORDER BY MissionCount ASC) AS LeastExperiencedSpacemanID
FROM Personal p
GROUP BY JobDescription
--hard 1
;WITH cte AS (
    SELECT 1 AS n, SUBSTRING('tf56sd#%OqH', 1, 1) AS ch
    UNION ALL
    SELECT n + 1, SUBSTRING('tf56sd#%OqH', n + 1, 1)
    FROM cte
...
SELECT 
    ...
    STRING_AGG(CASE WHEN ch NOT LIKE '[A-Za-z0-9]' THEN ch END, '') AS OtherChars
FROM cte
...
FROM Students;
--3 hard
select cast(replace(replace(equation, '+' ,0 ),'-',0)as int) from Equations

--4 hard
SELECT  StudentName, s.Birthday FROM Student s
JOIN (
    SELECT Birthday
    FROM Student
    GROUP BY Birthday
