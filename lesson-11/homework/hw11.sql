...
select ord.OrderID, Cus.FirstName+' '+cus.LastName as CustomerName, ord.OrderDate  from Orders ord
...
--3
select dep.DepartmentName,  max(salary) as Max_salary from Employees emp
join Departments dep on emp.DepartmentID=dep.DepartmentID
group by dep.DepartmentName

--4
select Cus.FirstName+' '+cus.LastName as CustomerName, ord.OrderID, ord.OrderDate from Customers cus
join Orders ord on ord.CustomerID=cus.CustomerID and cus.Country='USA'
where year(ord.OrderDate)=2023

--5
select Cus.FirstName+' '+cus.LastName as CustomerName, Quantity as TotalOrders  from Orders ord
join Customers cus on cus.CustomerID=ord.CustomerID

--6
select pro.ProductName, sup.SupplierName from Products pro
join Suppliers sup on pro.SupplierID=sup.SupplierID
where sup.SupplierName in ('Gadget Supplies' , 'Clothing Mart')

--7
select cus.FirstName,cus.LastName, max(orderDate) as RecentOrder from Customers cus
left join Orders ord on ord.CustomerID=cus.CustomerID
group by cus.FirstName, cus.LastName
 
 --8
 select Cus.FirstName,cus.LastName, ord.TotalAmount from Orders ord
 join Customers cus on ord.CustomerID=cus.CustomerID
 where TotalAmount>500

 --9
 select ProductName, sal.SaleDate, sal.SaleAmount  from Products pro
 join Sales sal on sal.ProductID=pro.ProductID
where sal.SaleAmount>400 or year(Sal.saleDate)=2022

--10
select pro.ProductName, coalesce(sum(sal.SaleAmount),0) as total_sale_amount from Products pro
left join Sales sal on pro.ProductID=sal.ProductID
group by pro.ProductName
order by total_sale_amount desc

--11
select emp.Name, dep.DepartmentName, Salary from Employees emp
join Departments dep on dep.DepartmentID=emp.DepartmentID
where DepartmentName='Human Resources' and Salary>60000

--12
select pro.ProductName, SaleDate, StockQuantity from Products pro
join Sales sal on pro.ProductID=sal.ProductID 
where year(SaleDate)=2023 and StockQuantity>100
 --13
 select emp.Name, DepartmentName,HireDate from Employees emp
 join Departments dep on dep.DepartmentID=emp.DepartmentID
where DepartmentName='Sales' and year(HireDate)>2020

--hard Level
--14
select Cus.FirstName+' '+LastName as Customer_name, OrderID,Address, OrderDate  from Customers cus
join Orders ord on cus.CustomerID=ord.CustomerID
where Country='USA' and Address like '[0-9][0-9][0-9][0-9]%'

--15
select ProductName, CategoryName,SaleAmount from Products pro
join Sales sal on sal.ProductID=pro.ProductID
join Categories on pro.Category=Categories.CategoryID
where Category=1 or SaleAmount>350
--16
select ctg.CategoryName, count(Category) as ProductCount from Products pro
join Categories Ctg on pro.Category=ctg.CategoryID
group by CategoryName

--17
select cus.FirstName+' '+LastName as CustomerName, City, ord.OrderID, TotalAmount  from Customers cus
join Orders ord on cus.CustomerID=ord.CustomerID
where City='Los Angeles' and TotalAmount>350
...
(len(Name)-len(replace(lower(Name), 'e','')))>=4
)

--19
select Name, DepartmentName, Salary from Employees emp
join Departments dep on dep.DepartmentID=emp.DepartmentID and emp.Salary>60000
