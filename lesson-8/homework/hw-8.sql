...
--1- usul. Using Products table, show the highest price among all products.
select top 1 ProductName, max(Price) as Max_price from Products
group by ProductName
order by Max(Price) Desc
--2-usul. Using Products table, show the highest price among all products.
select ProductName,Price from Products
where Price>=all (select price from Products
where Price>0)
select*,
case when StockQuantity<30 then 'Low Stock'
else 'Sufficinet' end as Status
from Products
select Country, count(CustomerID) as Customer_num from Customers
group by Country
CREATE TABLE Orders (
...
where i.CustomerID=o.CustomerID and
year(i.InvoiceDate)=2023 and
month(i.InvoiceDate)=1
)
...
union
select ProductName from Products_Discounted
Select year(OrderDate) as Per_year, avg(TotalAmount) Avarage_by_year from Orders
