...
--3
Create table  Person1 (id int, email varchar(255)) Truncate table Person 
...
select email as duplicateEmail from Person1
group by email
having count(email)>1

--4
Create table  Person2 (id int primary key, email varchar(255)) 
...
DELETE FROM CTE
WHERE rn > 1;   -- birinchi qatorni qoldirib, qolgan dublikatlarni o‘chiradi
select * from Person2
...
GO
	...
	JOIN Sales.OrderDetails AS OD
		ON O.orderid = OD.orderid
	WHERE OD.qty > 50   -- faqat vazni > 50 bo‘lgan buyurtmalar
	GROUP BY O.custid;
...
GO
select isnull(c1.Item ,' ') as Item,isnull(c2.Item ,' ') as Item from Cart1 c1
...
left join Examinations ex on ex.student_id=sdn.student_id and sbj.subject_name=ex.subject_name
group by sdn.student_name, sbj.subject_name
