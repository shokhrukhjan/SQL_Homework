...
--INTERSECT faqat ikkala query natijasida ham mavjud bo‘lgan satrlarni qaytaradi. Dublikatlar avtomatik olib tashlanadi (DISTINCT ishlaydi). Ustunlar soni va ularning datatypelari ikkala queryda ham bir xil bo‘lishi kerak.
select distinct FirstName, LastName, Country from Customers
select *,
 Case when Price>1000 then 'High'
 else 'Low' end as Status from Products
 Select*,
...
else 'Low Tier' end as Status from Sales
CREATE TABLE Orders (
