...
Select Cus.FirstName, Cus.LastName, Ord.OrderDate from Customers Cus
...
select sup.SupplierName, Pro.ProductName from  Products pro
Right Join Suppliers sup on pro.SupplierID=sup.SupplierID

select ord.OrderID, ord.OrderDate, pay.Paymentdate, pay.Amount from orders ord
Full Join Payments pay on ord.OrderID=pay.OrderID
...
select sdn.Name, crs.CourseName from Students sdn
join
Enrollments enr on enr.StudentID=sdn.StudentID
join
Courses crs on crs.CourseID=enr.CourseID
where CourseName='Math 101'

Select cus.FirstName, cus.LastName, ord.quantity  from Customers cus
join
...
join
 ...
 -- medium
 Select dep.DepartmentName, count(Emp.DepartmentID) as Employee_quantity from Departments dep 
...
select cus.FirstName, cus.LastName, ord.Quantity as TotalOrders from Customers Cus
left Join Orders Ord on Cus.CustomerID=Ord.CustomerID
where Ord.Quantity is not null

select Emp.Name as EmployeeName, dep.DepartmentName from Employees emp
left join
 Departments dep on emp.DepartmentID=dep.DepartmentID
 where dep.DepartmentID is not null
...
JOIN Employees e2 
	...
	--15
	select ord.OrderID, ord.OrderDate, Cus.FirstName+' ' +Cus.LastName as CustomerName from Orders ord
	join Customers Cus on ord.CustomerID=ord.CustomerID
 ...
 join 
	...
	--17
	select ord.OrderID, OrderDate, PaymentDate, Amount from  Orders ord
	 left join
	 Payments pay on ord.OrderID=pay.OrderID
	 where pay.PaymentDate is not null

	 --18
	 Select pro.ProductID, Pro.ProductName from Products pro
	left Join
	  Orders ord on pro.ProductID=ord.ProductID
...
left join
 Payments pay on pay.OrderID=ord.OrderID
 where ord.OrderDate<'2020-01-01' and pay.PaymentID is null
 
 --21
 select pro.ProductID, pro.ProductName from Products pro
 Left join Categories cat on Cat.CategoryID=Pro.Category
 where cat.CategoryName is null
...
Join Employees E2 on e1.ManagerID=e2.ManagerID
 ...
 join
  ...
  --24
  select SaleID,pro.ProductName,Sal.SaleAmount from Sales sal
  Join Products pro on pro.ProductID=sal.ProductID
  where SaleAmount>500

  --25
  select sdn.StudentID, sdn.Name from Students sdn
  join
   Enrollments enr on sdn.StudentID=enr.StudentID
  join Courses crs on crs.CourseID=enr.CourseID
  where crs.CourseName not in ('Math 101')

  --26
  select ord.OrderID, ord.OrderDate, PaymentID from Orders ord
  left join Payments pay on ord.OrderID=pay.OrderID
  where Pay.PaymentID is null

  --27
  Select ProductID, ProductName, CategoryName from Products pro
  left join
   Categories cgs on pro.Category=cgs.CategoryID
   where cgs.CategoryName in ('Electronics', 'Furniture')
