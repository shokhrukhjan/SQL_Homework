...
select count(*) from Customers -- bu yerda count(*) o'rniga count(CustomerID) deb kiritishimiz ham mumkin edi. Rowlar soni so'ralgani uchun count(*) bilan kiritdim
...
select DepartmentName, count(EmployeeID) as Number_of_emp from Employees
group by DepartmentName
select Category, min(Price) as Min_Price, max(Price) as Max_Price from Products
group by Category
select CustomerID, sum(SaleAmount) as Total_sale_Amount_by_per_customer from Sales
group by CustomerID
select DepartmentName, count(EmployeeID) from Employees
group by DepartmentName
having count(EmployeeID)>5
select ProductID, sum(SaleAmount) as Total_sale, avg(SaleAmount) Avarage_Sale from Sales
group by ProductID
select DepartmentName, count(EmployeeID) as Num_of_emp_HR from Employees
where DepartmentName='HR'
group by DepartmentName
select DepartmentName, max(Salary) as Max_salary, min(salary) as Low_Salary from Employees
group by DepartmentName
select DepartmentName, avg(Salary) as Avarage_salary from Employees
group by DepartmentName
select DepartmentName, avg(Salary) as Avarage_salary, count(EmployeeID) as Num_of_emp from Employees
group by DepartmentName
Select category, avg(Price) as AVG_PRICE from Products
group by Category
having avg(price)>400
select year(SaleDate) as Sales_per_year, sum(SaleAmount) as total_sale from Sales
...
select CustomerID, sum(quantity) as Quantity from Orders
group by CustomerID
...
select * from Employees
select departmentname, avg(salary) as Avarage_salary, sum(salary) as Total_salary from Employees
group by DepartmentName
...
RETURN
  ...
  ORDER BY rownum;
GO
	select custid, sum(freight) as total_freight, min(freight) as min_freight from TSQL2012.sales.orders
	where freight>50
	group by custid
select  year(OrderDate) as SalesYear,  MONTH(OrderDate) as Sales_per_month, sum(TotalAmount) as Total_sale, count(distinct ProductID) as UniqueProducts from orders
group by year(OrderDate), MONTH(OrderDate)
having count(distinct ProductID) >=2
order by SalesYear,Sales_per_month
select*from orders
select year(orderdate) as Per_year, max(quantity) as Max_Quantity, min(Quantity) as Min_Quantity from orders
group by Year(orderdate)
