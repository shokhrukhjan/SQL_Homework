...
select FirstName,LastName, o.OrderID from Customers C
Join 
Orders o
...
select S.Name, ENR.CourseID from Students S
join
Enrollments ENR
...
select Cus.FirstName, Cus.LastName, Sal.ProductID from Customers Cus
Join
Sales Sal
...
Payments Pay
on O.OrderID=Pay.OrderID
where Pay.Amount<O.TotalAmount
...
select S.SaleID,S.ProductID, S.CustomerID, S.SaleDate , S.SaleAmount from  Sales S
join
Customers C
...
select p.PaymentID, p.OrderID, p.PaymentDate, p.Amount, P.PaymentMethod  from Payments p
join orders o on p.OrderID=o.OrderID
join Products Pro on Pro.ProductID=o.ProductID
 where pro.Price*O.Quantity<>O.TotalAmount
